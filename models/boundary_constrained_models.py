...
class unet3d_mtl_tsol(nn.Module):
    def __init__(self, in_ch=1,out_ch=9):
        super(unet3d_mtl_tsol, self).__init__()
    ...
    def forward(self, x):
        ...
        output0_4_e = self.final_edge(x0_1)
...
class unetplus3d_mtl_tsol(nn.Module):
    def __init__(self, in_ch=1,out_ch=9):
        super(unetplus3d_mtl_tsol, self).__init__()
    ...
    def forward(self, x):
        ...
        output0_4_e = self.final_edge(x0_4)

        return output0_4, output0_4_e

class unetplus3d_mtl_tsd(nn.Module):
    def __init__(self, in_ch=1,out_ch=9):
        super(unetplus3d_mtl_tsd, self).__init__()
        ...
        self.conv3_1_e = DoubleConv_3d(filters[4] + filters[3], filters[3])
        self.conv2_2_e = DoubleConv_3d(filters[3] + filters[2], filters[2])       
        self.conv1_3_e = DoubleConv_3d(filters[2] + filters[1], filters[1])
        self.conv0_4_e = DoubleConv_3d(filters[0] + filters[1], filters[0])
    ...
    def forward(self, x):
        ...
        x3_1_e = self.conv3_1_e(torch.cat([x3_0, self.Up(x3_0,x4_0)], 1))
        x2_2_e = self.conv2_2_e(torch.cat([x2_0, self.Up(x2_0,x3_1_e)], 1))
        x1_3_e = self.conv1_3_e(torch.cat([x1_0, self.Up(x1_0,x2_2_e)], 1))
        x0_4_e = self.conv0_4_e(torch.cat([x0_0, self.Up(x0_0,x1_3_e)], 1))

        output0_4_e = self.final_edge(x0_4_e)
...
class attenunet3d_mtl_tsol(nn.Module):
    ...
    def __init__(self, out_ch=9, in_ch=1,
        ...
        self.final_edge = nn.Conv3d(filters[0], 1, 1)
    ...
    def forward(self, inputs):
        ...
        final_edge=self.final_edge(up1)
        return final, final_edge

class attenunet3d_mtl_tsd(nn.Module):

    def __init__(self, out_ch=9, in_ch=1,
                 nonlocal_mode='concatenation', attention_dsample=(2,2,2)):
        super(attenunet3d_mtl_tsd, self).__init__()
        filters = [16,32,64,128,256]
        self.pool = nn.MaxPool3d(kernel_size=2)
        ...
        self.up_concat4_e = attenunet_up(filters[4], filters[3], [0,0,0,0,0,0])
        self.up_concat3_e = attenunet_up(filters[3], filters[2], [0,0,0,0,0,0])
        self.up_concat2_e = attenunet_up(filters[2], filters[1], [0,0,0,0,0,0])
        self.up_concat1_e = attenunet_up(filters[1], filters[0], [0,0,0,0,0,0])
        self.final_edge = nn.Conv3d(filters[0], 1, 1)
    ...
    def forward(self, inputs):
        ...
        up4_e = self.up_concat4_e(conv4, center)
        up3_e = self.up_concat3_e(conv3, up4_e)
        up2_e = self.up_concat2_e(conv2, up3_e)
        up1_e = self.up_concat1_e(conv1, up2_e)
        final_edge=self.final_edge(up1_e)
        return final, final_edge
