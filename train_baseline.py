...
def main():
    parser = argparse.ArgumentParser(description='Single_task_model')
    ...
    parser.add_argument('--loss', default='dice_loss', type=str,
                    help='path to output folder for saving the results')
    ...
    if args.loss == "dice_loss":
        ...
        criterion = CE_loss() ## cross entropy loss
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(idx_epoch.size(1)):
            ...
            with torch.no_grad():
                if args.aug:
                    ...
                    imgs_cuda, y_label = imgs[idx,:,:,:,:].cuda(), segs[idx,:,:,:].cuda()
            ...
            optimizer.step()
            del loss; del predict; del imgs_cuda; del y_label_dc
            torch.cuda.empty_cache()
        scheduler.step()
        
        #evaluation on validation images
        t1 = time.time()-t0
        print('epoch',epoch,'time train','%.3f'%t1,'total_loss','%.4f'%(run_loss[epoch]))
        ...
        if mean_dice>high_val:
            ...
            torch.save(net.state_dict(), os.path.join(args.output_folder, "Best_" + args.model+ ".pth"),pickle_module=dill)
    ...
    net.load_state_dict(torch.load(os.path.join(args.output_folder, "Best_" + args.model+ ".pth")))
    net.eval()
